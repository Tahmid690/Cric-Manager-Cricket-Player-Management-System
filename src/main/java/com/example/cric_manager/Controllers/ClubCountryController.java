...
//        Salary.setCellValueFactory(new PropertyValueFactory<>("salary"));
    ...
    public void show_table(String clb,String coun) throws Exception {
        data= FXCollections.observableArrayList();
        //Add Products
        SocketWrapper socketWrapper = main.getSocketWrapper();
        socketWrapper.write("any");

        List<Player> out =  ((CricketPlayerDatabase)socketWrapper.read()).searchClub(coun,clb);
        for(Player p : out){
            data.add(p);
    ...
    public void sentclubname(ActionEvent actionEvent) throws Exception {
        String clb = clubname.getText();
        String country = countryname.getText();
        System.out.println(clb + " " + country);
        show_table(clb,country);
    }
    @FXML
