...
//        Salary.setCellValueFactory(new PropertyValueFactory<>("salary"));
    ...
    public void show_table(int lo,int hi) throws Exception {
        data= FXCollections.observableArrayList();
        //Add Products
        SocketWrapper socketWrapper = main.getSocketWrapper();
        socketWrapper.write("any");
        List<Player> out =  ((CricketPlayerDatabase)socketWrapper.read()).SalaryRange(lo,hi);
        for(Player p : out){
            data.add(p);
...
//        loadcol();
    ...
    public void sentclubname(ActionEvent actionEvent) throws Exception {
        int lo = Integer.parseInt(salo.getText());
        int hi = Integer.parseInt(salhi.getText());
